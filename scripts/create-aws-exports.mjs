...
const getStackName = async () => {
  try {
    ...
    const stack = res.StackSummaries.find((stack) =>
      stack.StackName.toUpperCase().includes("assettracker".toUpperCase())
    );
    if (!stack) {
    ...
    geo: {
      AmazonLocationService: {
        maps: {
          ...
          default: "AssetTrackerMap",
        },
        geofenceCollections: {
          items: ["AssetTrackerGeofenceCollection"],
          default: "AssetTrackerGeofenceCollection",
        },
        routeCalculator: "AssetTrackerRouteCalculator",
      },
    },
  };
